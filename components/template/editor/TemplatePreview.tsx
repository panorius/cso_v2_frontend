      ...
      <Card className="overflow-hidden">
        ...
        >
          ...
          {template.description && (
            <p className="mt-4 opacity-80">{template.description}</p>
          )}
        </div>

        {/* Content */}
        <div className="p-8" style={{ backgroundColor: template.theme.backgroundColor }}>
          {template.sections.length === 0 ? (
            <div className="py-12 text-center text-gray-400">
              <p>Aucune section définie</p>
              <p className="mt-2 text-sm">Ajoutez des sections pour voir l'aperçu</p>
            </div>
          ) : (
              ...
              {template.sections.map((section) => (
                <div key={section.id} className="space-y-4">
                  ...
                  >
                    ...
                    <Badge variant="flat" className="ml-auto">
                  ...
                  <div className="pl-4 space-y-3">
                    {section.modules.length === 0 ? (
                      <p className="py-4 text-sm italic text-gray-400">
                        Section vide - Ajoutez des modules
                      </p>
                    ) : (
                      section.modules.map((module, idx) => (
                        ...
                        >
                          ...
                          <Badge variant="flat" className="ml-auto text-xs">
                            Module {module}
                          </Badge>
                        </div>
                      ))
        ...
        <div className="px-8 py-4 border-t bg-gray-50">
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <span>
              {template.sections.reduce((acc, section) => acc + section.modules.length, 0)}{' '}
              modules au total
            </span>
          </div>
        </div>
